...
#### 토큰 생성/소멸 정책 (엔진 구현 기준)

* 토큰 생성은 공통 API(`ZoneOps.createTokenInZone`)를 통해서만 수행한다.
* 생성 가능 존은 HAND/FIELD로 제한한다.
* HAND 생성 시 손패가 가득 찼다면(`hand.size >= handLimit`) 생성을 거부하고 로그 이벤트를 남긴다.
* FIELD 생성 시 필드가 가득 찼다면(`field.size >= fieldLimit`) 생성을 거부하고 로그 이벤트를 남긴다.
* 생성된 토큰이 DECK/GRAVE/EXCLUDED/EX로 이동하려고 하면 기존 규칙(`moveToZoneOrVanishIfToken`)에 따라 소멸한다.
...
* 사용자는 캐릭터 프로필 + 덱(12장) + EX(1장) + 캐릭터 패시브(최대 2개)를 프리셋으로 저장/선택할 수 있다.
* AC: 카드 풀은 고정이며, 덱 규칙(장수/중복 제한 등)은 저장 시 검증된다.
* AC: 패시브는 0~2개만 허용되며, 각 ID는 `P###` 포맷(예: `P001`)이어야 하고 중복은 불가하다.
* AC: 존재하지 않는 패시브 ID는 거부되며, 오류 메시지는 사용자가 바로 수정할 수 있도록 원인을 명확히 안내한다.
* AC: 패시브는 `join` 최초 1회에만 확정되며(Join-only), 로비/전투 중 재설정은 허용되지 않는다.
...
## 12) API 커맨드 타입 정책 (`CommandRequest.type`)
