...
Combat Lab은 프론트에서 주사위나 카드 효과를 다시 계산하지 않습니다. 프론트는 백엔드 Lab API 응답을 표시하고, 계산과 검증은 서버의 기존 규칙과 CardEffect 구현을 사용합니다.

## 1. 개요

Combat Lab은 두 가지 도구로 구성됩니다.

- Dice API Tester: 주사위 표기를 서버 Dice API로 검증하고 굴림 결과, 기대값, 최소/최대값, 분포를 확인합니다.
...
- notation: 주사위 표기입니다. 예: `3d6+2`
- rollCount: 굴림 횟수입니다. 0이면 실제 굴림 없이 parse/min/max/expected만 확인합니다.
- seed: 선택 입력입니다. 같은 seed와 같은 요청이면 같은 rolls 재현에 사용할 수 있습니다.

결과 필드:

- spec: count, sides, modifier로 분해된 서버 해석 결과입니다.
- min/max: 가능한 최소값과 최대값입니다.
- expected: 기대값입니다. 프론트 표시를 위해 문자열로 내려옵니다.
- rolls: 서버가 실제로 굴린 결과 목록입니다.
- histogram: 서버가 계산한 value/count 분포입니다. 프론트는 이 값을 재계산하지 않습니다.

## 3. Effect Probe 사용법

Effect Probe 탭은 `/api/lab/effects/cards`와 `/api/lab/effects/probe`를 사용합니다.
...
- Validate: `CardEffect.validate`만 실행합니다. resolve는 실행하지 않으므로 before와 after가 같아야 합니다.
...
- Target HP / Actor HP: 서버 response.before/after와 response.changes를 기반으로 표시됩니다.
- Status changes: 서버 changes에 포함된 상태 변화 수입니다.
- Events: 서버가 반환한 GameEvent 목록입니다.
- Notes: Probe가 지원하거나 의도적으로 생략한 범위입니다.
- Raw JSON: request/response 원문입니다. 버그 리포트나 카드 밸런싱 기록에 복사하기 좋습니다.
...
- AP 비용 검사 없음
- 손패 소유 검사 없음
- 실제 덱/묘지/제외/필드 흐름 재현 없음
- PlayCardCommand의 사용 후 카드 이동 없음
- 턴 시작/턴 종료 처리 없음
...
주의: CardEffect 내부에서 직접 발생시키는 카드 이동이나 이벤트는 관찰될 수 있습니다. 다만 실전 카드 사용 전체 절차를 의미하지는 않습니다.
...
4. Resolve를 실행합니다.
5. Summary의 Target HP 변화와 Changes를 확인합니다.
...
하위 호환을 위해 기존 단일 `target` request도 계속 지원합니다. `target`과 `targets`가 함께 있으면 단일 `target`이 우선 처리되고, 이 사실은 response notes에 표시됩니다.
