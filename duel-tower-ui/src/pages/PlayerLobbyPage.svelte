<script lang="ts">
  ...
  function getSessionCodeFromRoute() {
    ...
    if (match?.page.key !== 'player-lobby') {
      return null
    }
  ...
  // TODO: Remove the legacy fixed lobby route and handoff fallback once /sessions/:code/player is the only entry path.
</script>
...
<div class="player-lobby-page">
  {#if session}
    <SectionFrame
      ...
      title={session.title}
      description="Player lobby now resolves the current session from the URL code first, with the old handoff kept only as a temporary fallback."
    >
    ...
    >
      <div class="player-lobby-page__actions">
        <a class="player-lobby-page__link-action" data-nav href={pathBuilders.sessionEntry()}>
    ...
    <SectionFrame
      ...
      description={
        missingSessionCode
          ...
          : 'Open a session from the entry screen or use a valid player lobby URL to restore this page.'
      }
    >
      <div class="player-lobby-page__todo">
        {#if missingSessionCode}
          <p>Requested code: {missingSessionCode}</p>
          <p>Check the session entry screen and open a valid player lobby.</p>
        {:else}
          <p>No session code was found in the URL, and no handoff selection is available.</p>
          <p>Open a session from the entry screen to restore the expected player lobby context.</p>
        {/if}
      </div>
      <div class="player-lobby-page__actions">
        <a class="player-lobby-page__link-action" data-nav href={pathBuilders.sessionEntry()}>
