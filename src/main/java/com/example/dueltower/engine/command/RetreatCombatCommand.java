...
public record RetreatCombatCommand(
        UUID commandId,
    ...
    public List<String> validate(GameState state, EngineContext ctx) {
        ...
        if (state.combat() == null || state.nodeState() != NodeState.COMBAT) {
            errors.add("retreat is only available during combat");
        }
        return errors;
    }
    ...
    public List<GameEvent> handle(GameState state, EngineContext ctx) {
        ...
        return List.of(new GameEvent.LogAppended(playerId.value() + " retreats from combat." + detail));
    }
}
